(function() {
    ...
    loadData: function(filter) {
            $.ajax({
                ...
                data : filter
                    ...
                    return $.grep(this.jobs, function(job) {
                        return (!filter.jobFunction || job.Name.indexOf(filter.Name) > -1)
                            && (!filter.jobType || job.jobType.indexOf(filter.jobType) > -1)
                            && (!filter.location || job.location.indexOf(filter.location) > -1);
                })
        ...
        insertItem: function(insertingClient) {
            this.jobs.push(insertingClient);
        },

        updateItem: function(updatingClient) { },

        deleteItem: function(deletingJob) {
            var clientIndex = $.inArray(deletingJob, this.jobs);
            this.jobs.splice(clientIndex, 1);
        }
